...
def find_time_column(df):
    for column in df:
        if 'time' in column.lower():
            return column
    raise Exception('No column found for time. Try passing in a column name with the "time_column" argument')
...
def nrft(
    ...
    all_aggregates_total['result'] = (all_aggregates_total['result']/all_aggregates_total[aggregate_column])*100
...
def qty(
    ...
    if len(aggregate_columns) == 1:
        output_data = {
            x_column: [data for data in all_aggregates_total.index]
        }
    else:
        output_data = {
    ...
    output_data.update(
        {
            'result': [value for value in all_aggregates_total]
        }
    )
