...
class Table(Plot):
    def __init__(
        ...
        **kwargs
        ...
        if isinstance(data_source, pd.DataFrame):
            ...
            raise Exception(
                "Data source not valid, please pass in a dataframe or function"
            )

        super().__init__(
            data_source, post_process_func, colorway, style, static_data_args, **kwargs
        )
    ...
    def _build_dash_component(self, **kwargs):
        ...
        self.dash_component = dash_table.DataTable(
            id=self._id,
            columns=[
                {"name": column, "id": column} for column in self._output_data.columns
            ],
